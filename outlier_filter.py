...
class OutlierFilter:
    def __init__(self, max_transactions=360, max_monetary=60000, max_item_variety=500,
                 min_transactions=20, min_monetary=1000):
    ...
    def filter_outliers(self, data, save_path="filtered_data.csv"):
        ...
        Also filters out customers with fewer than the minimum required transactions or spending.
        ...
        filtered_data = data[
            ...
            (data['TotalTransactions'] >= self.min_transactions) &  # Keep >= 20 transactions
            (data['MonetaryValue'] <= self.max_monetary) &          # Keep <= 60,000 TL spent
            (data['MonetaryValue'] >= self.min_monetary) &          # Keep >= 1,000 TL spent
